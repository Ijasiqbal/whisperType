#!/bin/bash
#
# Vozcribe macOS — Build, Sign, Package, Notarize
#
# Prerequisites:
...
ARCHIVE_PATH="$BUILD_DIR/Vozcribe.xcarchive"
...
if [ "$HAS_DEV_ID" = true ]; then
    ...
    xcodebuild -exportArchive \
        ...
        -quiet
    APP_PATH="$EXPORT_PATH/Vozcribe.app"
    if [ ! -d "$APP_PATH" ]; then
    ...
    APP_PATH="$ARCHIVE_PATH/Products/Applications/Vozcribe.app"
    if [ ! -d "$APP_PATH" ]; then
        # Fallback to derived data path
        APP_PATH="$(find "$ARCHIVE_PATH" -name "Vozcribe.app" -type d | head -1)"
    fi
    if [ ! -d "$APP_PATH" ]; then
...
DMG_NAME="Vozcribe-${DMG_VERSION}.dmg"
...
if [ "$HAS_CREATE_DMG" = true ] && [ -f "$BG_IMAGE" ]; then
    ...
    hdiutil create -volname "Vozcribe" -srcfolder "$DMG_DIR" -ov -format UDZO -imagekey zlib-level=9 "$DMG_PATH"
fi
...
echo ""
echo -e "${GREEN}════════════════════════════════════════${NC}"
echo -e "${GREEN}  Vozcribe $DMG_VERSION — Ready for distribution${NC}"
echo -e "${GREEN}════════════════════════════════════════${NC}"
echo ""
echo "  DMG: $DMG_PATH"
